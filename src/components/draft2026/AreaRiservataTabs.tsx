...
import { Play, Radio, Sparkles, Lock, ChevronDown, Lightbulb } from "lucide-react";
    ...
    return (
        <div className="w-full max-w-4xl mx-auto mt-6 mb-4 md:mb-24 px-4 overflow-x-hidden">
            <div className="flex flex-col border-t border-white/10">
                ...
                {(profile?.plan_type === 'free_trial' || profile?.plan_type === 'basic') && (
                    <div className="border-b border-white/10">
                        ...
                        >
                             <div className="flex items-center gap-4">
                                {profile?.plan_type === 'basic' ? (
                                    <Lightbulb className={`w-5 h-5 ${openSection === 'potenzia' ? 'text-[#ff7393]' : 'text-zinc-400 group-hover:text-[#ff7393]'}`} />
                                ) : (
                                ...
                                <span className={`text-lg md:text-xl font-medium tracking-wide transition-colors ${openSection === 'potenzia' ? (profile?.plan_type === 'basic' ? 'text-[#ff7393]' : 'text-purple-400') : `text-zinc-400 ${profile?.plan_type === 'basic' ? 'group-hover:text-[#ff7393]' : 'group-hover:text-purple-400'}`}`}>
                                    {profile?.plan_type === 'basic' ? 'Come sfruttare il servizio BeautiFy' : 'Potenzia il Servizio'}
                                </span>
                            </div>
                        ...
                        <div className={`px-4 py-4 md:px-6 lg:px-12 md:py-8 bg-gradient-to-b ${profile?.plan_type === 'basic' ? 'from-[#ff7393]/[0.02]' : 'from-[#ff5a7e]/[0.02]'} to-transparent rounded-2xl border border-white/5 relative`}>
                            <div className={`${profile?.plan_type === 'basic' ? 'w-full' : 'grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16'}`}>
                                ...
                                <div className="text-base md:text-lg font-light leading-relaxed text-zinc-300 flex flex-col justify-center">
                                    ...
                                    <ul className="mb-8 space-y-4 text-sm md:text-base">
                                        <li className="flex items-start gap-3">
                                            <div className={`w-1.5 h-1.5 rounded-full ${profile?.plan_type === 'basic' ? 'bg-[#ff7393]' : 'bg-purple-400'} mt-2 shrink-0`} />
                                ...
                                {/* RIGHT COLUMN: STAY TUNED (Only for Free Trial - For Basic it is outside) */}
                ...
                {/* STANDALONE STAY TUNED BLOCK FOR BASIC USERS ONLY (EXTRACTED FROM THE ACCORDION) */}
                {profile?.plan_type === 'basic' && (
